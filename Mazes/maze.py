        ...
        for y in range(6):  # for each x (6)
            ...
            for x in range(5):  # for each x (5) because there are only 5 columns  of vertical walls
                ...
                vertical_wall[y].append(
                    bool(input(f"is there a vertical wall after {x + 1, y + 1}")))  # adding 1 to x and y so as the
...
class Maze:
    def __init__(self, data: dict, win):
        ...
        self.screen = win  # pygame scree to draw and do stuff
    ...
    def draw_path(self, route: list[list[int]]) -> None:
        print(route)
        for lis in range(len(route)):
            x, y = route[lis]
